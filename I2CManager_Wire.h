...
// ---------------------------------------------------------------------------
// I2C debug logging
// Set to 0 to disable all Serial debug messages from this file.
// ---------------------------------------------------------------------------
#ifndef I2C_WIRE_DEBUG
#define I2C_WIRE_DEBUG 1
#endif

#if I2C_WIRE_DEBUG
  #define I2CDBG(...) Serial.printf(__VA_ARGS__)
#else
  #define I2CDBG(...)
#endif
...
static uint8_t muxSelect(I2CAddress address) {
  ...
  if (muxNo != I2CMux_None) {
    uint8_t muxAddress = I2C_MUX_BASE_ADDRESS + muxNo;
...
#endif
    Wire.write(&data, 1);
    uint8_t muxStatus = Wire.endTransmission(true);  // have to release I2C bus for it to work
    if (muxStatus != I2C_STATUS_OK) {
      I2CDBG("I2C MUX fail mux=0x%02X subBus=%u status=%u\n",
             muxAddress, (unsigned)subBus, (unsigned)muxStatus);
    }
    return muxStatus;
  }
...
uint8_t I2CManagerClass::write(I2CAddress address, const uint8_t buffer[], uint8_t size, I2CRB *rb) {
  ...
  do {
    ...
    if (muxStatus == I2C_STATUS_OK && address != 0) {
      I2CDBG("I2C WRITE start addr=0x%02X size=%u retry=%u\n",
             (unsigned)address.deviceAddress(), (unsigned)size, (unsigned)retryCount);
      ...
      if (status != I2C_STATUS_OK) {
        I2CDBG("I2C WRITE fail addr=0x%02X status=%u size=%u retry=%u\n",
               (unsigned)address.deviceAddress(), (unsigned)status,
               (unsigned)size, (unsigned)retryCount);
      } else {
        I2CDBG("I2C WRITE ok addr=0x%02X size=%u retry=%u\n",
               (unsigned)address.deviceAddress(), (unsigned)size, (unsigned)retryCount);
      }
    }
#ifdef I2C_EXTENDED_ADDRESS
                              ...
                              const uint8_t writeBuffer[], uint8_t writeSize, I2CRB *rb)
{
  ...
  do {
    ...
    if (muxStatus == I2C_STATUS_OK && address != 0) {
      I2CDBG("I2C READ start addr=0x%02X wsize=%u rsize=%u retry=%u\n",
             (unsigned)address.deviceAddress(), (unsigned)writeSize,
             (unsigned)readSize, (unsigned)retryCount);
      ...
      if (writeSize > 0) {
        ...
        if (status != I2C_STATUS_OK) {
          I2CDBG("I2C READ-SETUP fail addr=0x%02X status=%u wsize=%u retry=%u\n",
                 (unsigned)address.deviceAddress(), (unsigned)status,
                 (unsigned)writeSize, (unsigned)retryCount);
        }
      }
      if (status == I2C_STATUS_OK) {
        ...
        if (!Wire.getWireTimeoutFlag()) {
          ...
          if (nBytes < readSize) {
            ...
            I2CDBG("I2C READ truncated addr=0x%02X got=%u expected=%u retry=%u\n",
                   (unsigned)address.deviceAddress(), (unsigned)nBytes,
                   (unsigned)readSize, (unsigned)retryCount);
          } else {
            I2CDBG("I2C READ ok addr=0x%02X got=%u retry=%u\n",
                   (unsigned)address.deviceAddress(), (unsigned)nBytes,
                   (unsigned)retryCount);
          }
        } else {
          status = I2C_STATUS_TIMEOUT;
          I2CDBG("I2C READ timeout addr=0x%02X expected=%u retry=%u\n",
                 (unsigned)address.deviceAddress(), (unsigned)readSize,
                 (unsigned)retryCount);
        }
#else
        Wire.requestFrom(address, (size_t)readSize);
