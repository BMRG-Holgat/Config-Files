...
void I2CManagerClass::I2C_handleInterrupt() {
  ...
  if (temp_sr1 & (I2C_SR1_AF | I2C_SR1_ARLO | I2C_SR1_BERR)) {
    DIAG(F("I2C error SR1=%04X device=0x%02X state=%d"),
       temp_sr1, deviceAddress, transactionState);
       Serial.printf("I2C error SR1=%04X device=0x%02X\n", temp_sr1, deviceAddress);
    ...
    switch (transactionState) {
      case TS_START:
        if (temp_sr1 & I2C_SR1_SB) {
          DIAG(F("I2C start dev=0x%02X op=%d"), deviceAddress, operation);
          Serial.printf("I2C error SR1=%04X device=0x%02X\n", temp_sr1, deviceAddress);
          // Event EV5
